...
class DogsDataSet_Train(Dataset):
    def __init__(self, dataType='train'):
        # Reading attributes
        self.dogList = pd.read_csv('attributes_with_closest.csv')
        self.dogList = self.dogList[self.dogList['type'] == dataType]
        self.dogList = self.dogList.reset_index(drop=True)

        # Converting most similar dogs string list to list of strings
        self.dogList['closestPaths'] = [ast.literal_eval(i) for i in self.dogList['closestPaths'].values.tolist()]
    ...
    def __getitem__(self, index):
        ...
        # Randomly selecting from the 5 most similar
        negativeDogPath = random.choice(self.dogList.iloc[index]['closestPaths'])

        # Getting the index
        negativeImgIndex = self.dogList[self.dogList['path'] == negativeDogPath].index.values[0]
